/mob/living/carbon/human/emote(var/act,var/m_type=1,var/message = null, var/cooldown = 2)
	...
	switch(act)
		if ("airguitar")
			if (!src.restrained())
				message = "<B>[src]</B> бренчит по воздуху и дергает головой, как дикая обезь[LOST_LETTER]на."
				m_type = 1
		...
		if ("custom")
			...
			if (!input)
				return
			var/input2 = input("Это видима[LOST_LETTER] или звукова[LOST_LETTER] эмоци[LOST_LETTER]?") in list("Visible","Hearable")
			if (input2 == "Visible")
				m_type = 1
				...
				alert("Невозможно использовать эту эмоцию, она должна быть видима[LOST_LETTER] или слышима[LOST_LETTER].")
				return
		...
		if ("johnny")
			...
			if (!M)
				...
				if(miming)
					message = "<B>[src]</B>  затягиваетс[LOST_LETTER] сигаретой и выдыхает дым \"[M]\" в дыму."
					m_type = 1
				else
					message = "<B>[src]</B> says, \"[M], please. He had a family.\" [src.name] затягиваетс[LOST_LETTER] сигаретой и выдувает свое им[LOST_LETTER]."
					m_type = 2
